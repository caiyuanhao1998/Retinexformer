...
class RetinexFormer_Single_Stage(nn.Module):
    ...
    def forward(self, img):
        ...
        input_img = img / (illu_map + 1e-4) + img
        output_img = self.denoiser(input_img)

        return output_img


class RetinexFormer_ablation_orf_div(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=31, stage=3, num_blocks=[1,1,1]):
        super(RetinexFormer_ablation_orf_div, self).__init__()
        self.stage = stage
...
#     model = RetinexFormer_ablation_orf_div(stage=1,n_feat=40,num_blocks=[1,2,2]).cuda()
#     print(model)
