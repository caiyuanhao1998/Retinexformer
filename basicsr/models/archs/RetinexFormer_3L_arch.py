...
class RetinexFormer_3L(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=31, stage=3, level=3, num_blocks=[1,1,1,1]):
        super(RetinexFormer_3L, self).__init__()
        self.stage = stage

        modules_body = [RetinexFormer_Single_Stage(in_channels=in_channels, out_channels=out_channels, n_feat=n_feat, level=level, num_blocks=num_blocks)
...
#     model = RetinexFormer(stage=1,n_feat=40,level=2, num_blocks=[1,2,2]).cuda()
#     print(model)
