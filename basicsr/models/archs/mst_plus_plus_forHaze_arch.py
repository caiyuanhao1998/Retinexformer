    ...
    def __init__(
        ...
        for _ in range(num_blocks):
            self.blocks.append(nn.ModuleList([
                ...
                PreNorm(dim, FeedForward(dim=dim, mult=2))
            ]))
...
class MST_Plus_Plus(nn.Module):
    def __init__(self, in_channels=3, out_channels=31, n_feat=31, stage=3):
        ...
        modules_body = [MST(in_dim=n_feat, out_dim=n_feat, dim=n_feat, stage=2, num_blocks=[1, 1, 1])
...
#     model = MST_Plus_Plus(n_feat=40).cuda()
#     print(model)
