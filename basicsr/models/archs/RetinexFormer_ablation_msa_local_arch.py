...
class SW_MSA(nn.Module):
    ...
    def forward(self, x):
        ...
        # split heads
        B,N,C = q.shape
        q, k, v = map(lambda t: t.clone().view(B, N, self.heads, C // self.heads).permute(0, 2, 1, 3), (q, k, v))
...
class SWAB(nn.Module):
    def __init__(
        ...
        for _ in range(num_blocks):
            self.blocks.append(nn.ModuleList([
                PreNorm(dim,SW_MSA(dim=dim, window_size=window_size, heads=heads,
                                    shift_size=(0, 0) if (_ % 2 == 0) else (
...
class Denoiser(nn.Module):
    ...
    def forward(self, x, illu_fea):
        ...
        fea_encoder = []
        illu_fea_list = []
        for (SWAB, FeaDownSample, IlluFeaDownsample) in self.encoder_layers:
            fea = SWAB(fea,illu_fea)  # bchw
...
class RetinexFormer_ablation_msa_local(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=31, stage=3, num_blocks=[1,1,1]):
        super(RetinexFormer_ablation_msa_local, self).__init__()
        self.stage = stage
...
#     model = RetinexFormer_ablation_msa_local(stage=1,n_feat=40,num_blocks=[1,2,2]).cuda()
#     print(model)
