...
class RetinexFormer_Single_Stage(nn.Module):
    ...
    def forward(self, img):
        '''
            :output illu_fea    输出特征
            :output illu_map    光照估计图
            :output input_img   输入去噪网络的图像
            :output output_img  去噪网络输出的图像
        '''
        ...
        return illu_fea, illu_map, input_img, output_img 


class RetinexFormer_vis(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=31, stage=3, num_blocks=[1,1,1]):
        super(RetinexFormer_vis, self).__init__()
        self.stage = stage
    ...
    def forward(self, x):
        ...
        return out:[b,c,h,w]
        """
        illu_fea, illu_map, input_img, output_img  = self.body(x)
...
#     model = RetinexFormer(stage=1,n_feat=40,num_blocks=[3,5,8]).cuda()
#     print(model)
