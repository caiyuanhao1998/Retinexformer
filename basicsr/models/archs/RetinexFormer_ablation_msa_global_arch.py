...
class GlobalAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        v_inp = x
        x = F.interpolate(x.permute(0,3,1,2),scale_factor=0.25).permute(0,2,3,1)
        b, h, w, c = x.shape
        ...
        out = F.interpolate(out.permute(0, 3, 1, 2), scale_factor=4).permute(0, 2, 3, 1)
        out_p = self.pos_emb(v_inp.permute(
            0, 3, 1, 2)).permute(0, 2, 3, 1)
        return out + out_p
...
class GLAB(nn.Module):
    def __init__(
        ...
        for _ in range(num_blocks):
            self.blocks.append(nn.ModuleList([
                PreNorm(dim,GlobalAttention(dim=dim,  heads=heads)),
                PreNorm(dim, FeedForward(dim=dim))
            ]))
    ...
    def forward(self, x, illu_fea):
        """
        x: [b,c,h,w]
        sparsity_mask: [b,1,h//w_size,w//w_size]
        return out: [b,c,h,w]
        """
        x = x.permute(0, 2, 3, 1)
        for (attn, ff) in self.blocks:
...
class Denoiser(nn.Module):
    def __init__(self, in_dim=3, out_dim=3, dim=31, level=2, num_blocks=[2, 4, 4]):
        ...
        for i in range(level):
            self.encoder_layers.append(nn.ModuleList([
                GLAB(
                    dim=dim_level, num_blocks=num_blocks[i], heads=dim_level // dim),
        ...
        # Bottleneck
        self.bottleneck = GLAB(
            dim=dim_level, heads=dim_level // dim, num_blocks=num_blocks[-1])
    ...
    def forward(self, x, illu_fea):
        ...
        fea_encoder = []
        illu_fea_list = []
        for (GLAB, FeaDownSample, IlluFeaDownsample) in self.encoder_layers:
            fea = GLAB(fea,illu_fea)  # bchw
...
class RetinexFormer_ablation_msa_global(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=31, stage=3, num_blocks=[1,1,1]):
        super(RetinexFormer_ablation_msa_global, self).__init__()
        self.stage = stage
...
#     model = RetinexFormer_ablation_msa_global(stage=1,n_feat=40,num_blocks=[1,2,2]).cuda()
#     print(model)
