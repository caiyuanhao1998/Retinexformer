...
class Illumination_Estimator(nn.Module):
    ...
    def forward(self, img):
        ...
        illu_fea = self.depth_conv(x_1) + x_1
...
class RetinexFormer_res(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=31, stage=3):
        super(RetinexFormer_res, self).__init__()
        self.stage = stage

        modules_body = [RetinexFormer_Single_Stage(in_channels=in_channels, out_channels=out_channels, n_feat=n_feat, level=2, num_blocks=[1, 1, 1])
...
#     model = RetinexFormer(n_feat=40).cuda()
#     print(model)
